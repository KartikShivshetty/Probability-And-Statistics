# Probability & Statistics — Academic Portfolio

Academic portfolio and course documentation for **Probability and Statistics** at **KLE Technological University**.

This repository reorganizes the original visual portfolio into a readable academic record. The repository contains chapter-wise notes, the mini project, laboratory documentation, and the original portfolio website.

## Student

- **Name:** Kartik Shivshetty
- **Program:** Computer Science and Engineering
- **Roll Number:** 632
- **USN:** 01FE25BCS387
- **University:** KLE Technological University
- **Course:** Probability and Statistics
- **Track:** Embedded + AI + Robotics

## Repository Contents

| Section | Description |
|---|---|
| [Course Overview](#course-overview) | Course and portfolio context |
| [Chapter 1 — Description of Data](./01-description-of-data/README.md) | Data, descriptive statistics, visualization, and formulas |
| [Chapter 2 — Probability](./02-probability/README.md) | Probability laws, conditional probability, Bayes theorem, and applications |
| [Chapter 3 — Random Variables & Probability Distributions](./03-random-variables-and-probability-distributions/README.md) | Random variables, distributions, statistical parameters, and applications |
| [Chapter 4 — Statistical Inference](./04-statistical-inference/README.md) | Sampling, hypothesis testing, confidence intervals, and inference formulas |
| [Mini Project — AI Impact on Job Market](./mini-project/README.md) | Statistical analysis of AI and employment data |
| [Laboratory](./laboratory/README.md) | Four documented Python/Jupyter experiments |
| [Original Portfolio](./web/index.html) | Original visual portfolio website source |

## Course Overview

The portfolio documents learning in probability and statistics through four chapters, practical laboratory work, and a mini project. The academic material covers descriptive statistics, probability, random variables and probability distributions, and statistical inference.

The practical work uses Python and Jupyter Notebook together with Pandas, NumPy, Matplotlib, Seaborn, and SciPy where documented by the portfolio.

## Mini Project

The portfolio's mini project is **AI Impact on Job Market**. It applies Probability and Statistics concepts to employment-related data, including salary, automation risk, remote work, and employment status. The documented analysis includes descriptive statistics, visualization, probability, hypothesis testing, correlation and regression, and chi-square testing.

[View the complete project documentation →](./mini-project/README.md)

## Laboratory

Four laboratory experiments are documented:

1. Python Basics & Control Structures
2. Statistical Analysis using E-Commerce Dataset
3. Probability Distribution & Binomial Analysis
4. Python Data Structures & Tuple Operations

[View laboratory documentation →](./laboratory/README.md)

## Original Visual Portfolio

The original interactive portfolio is preserved under [`web/`](./web/). It remains separate from the academic Markdown documentation so the visual presentation and the academic record serve different purposes.

[Open the portfolio source →](./web/index.html)

## External Academic Resources

The original portfolio contains external Google Drive notebook links for laboratory work and a Google Docs link for the mini-project report. Those links are preserved in the corresponding documentation instead of being replaced with fabricated local files.

## Repository Organization

```text
probability-and-statistics/
├── README.md
├── LICENSE
├── .gitignore
├── 01-description-of-data/
│   └── README.md
├── 02-probability/
│   └── README.md
├── 03-random-variables-and-probability-distributions/
│   └── README.md
├── 04-statistical-inference/
│   └── README.md
├── mini-project/
│   └── README.md
├── laboratory/
│   ├── README.md
│   ├── 01-python-basics-and-control-structures.md
│   ├── 02-statistical-analysis-e-commerce-dataset.md
│   ├── 03-probability-distribution-binomial-analysis.md
│   └── 04-python-data-structures-tuple-operations.md
└── web/
    └── index.html
```

## Notes on Source Fidelity

This repository is derived from the supplied portfolio source. Content has been reorganized for GitHub Markdown without inventing additional experiments, datasets, results, references, or academic sections.

Mathematical expressions have been normalized into GitHub-compatible LaTeX where the original HTML contained character-encoding artifacts.
